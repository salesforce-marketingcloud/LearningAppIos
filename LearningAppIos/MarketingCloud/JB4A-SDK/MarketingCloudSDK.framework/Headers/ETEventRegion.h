//
//  ETEventRegion.h
//  JB4A-SDK-iOS
//
//  Copyright © 2016 Salesforce. All rights reserved.
//

#import <Foundation/Foundation.h>
#import "PushConstants.h"

NS_ASSUME_NONNULL_BEGIN

/**
 ETEventRegion is a representation of a physical region that should trigger a message to be presented to the user. This could be either macro, a geofence, or micro, an iBeacon (marketing name: Proximity), but both go through iOS' Location Manager, and are reported back to the SDK through the CLLocationManagerDelegate (which is currently always ETLocationManager).
 
 Geofences will have a latitude and longitude and radius, but will be notably absent of a proximity UUID, major and minor number. Beacons also have latitude and longitude (but no radius) because of a decision to track their physical location in the world, but it is inconsequential to functionality. A beacon is functional because of the Proximity UUID. Major and Minor number. The three of these uniquely identify a physical beacon. Per Apple's best practices (WWDC 2013), a single UUID should be used commonly amongst an entire Enterprise (example: All Starbucks locations share a UUID). Major numbers should designate a single location within the UUID (Motes Coffee Shop #1234, 15th and College, Indianapolis), and a Minor number can indicate the beacon within the location designated in the Major (Point of Sale). Salesforce suggests following this pattern when configuring beacons.
 */

@interface ETEventRegion : NSObject


/**
 ET-generated identifier for the ETRegion in question. This should be treated as a primary key, and is stored on the device as the encoded version sent via the routes.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSString *fenceIdentifier __attribute__((deprecated("__REPLACEME__")));

/**
 The latitude of this region. Saved in an NSNumber as a double for easy passing. Be sure to call doubleValue on this property.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSNumber *latitude __attribute__((deprecated("__REPLACEME__")));

/**
 The longitude of this region. Saved in an NSNumber as a double for easy passing. Be sure to call doubleValue on this property.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSNumber *longitude __attribute__((deprecated("__REPLACEME__")));

/**
 For geofences only, the radius of the fence. This number, an integer, is in meters.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSNumber *radius __attribute__((deprecated("__REPLACEME__")));

/**
 For beacons, the Proximity UUID of the beacon.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSString *proximityUUID __attribute__((deprecated("__REPLACEME__")));

/**
 For beacons, the Major number. This is a uint32 per the CLBeaconRegion spec.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSNumber *majorNumber __attribute__((deprecated("__REPLACEME__")));

/**
 For beacons, the Minor number. This is a uint32 per the CLBeaconRegion spec.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSNumber *minorNumber __attribute__((deprecated("__REPLACEME__")));


/**
 This is the name which is set on SalesforceMarketingCloud, while setting the ETRegion
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSString *regionName __attribute__((deprecated("__REPLACEME__")));

/**
 Helper to quickly determine if this is a Geofence region.
 
 @return T/F if a geofence.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly) BOOL isGeofenceRegion __attribute__((deprecated("__REPLACEME__")));

/**
 Helper to quickly determine if this is a Beacon/Proximity region.
 
 @return T/F if a beacon.
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly) BOOL isBeaconRegion __attribute__((deprecated("__REPLACEME__")));

/**
 An array of event messages associated with this region
 @deprecated in __DOC_REPLACEME__
 */
@property (nonatomic, readonly, copy, nullable) NSArray *messages __attribute__((deprecated("__REPLACEME__")));



@end
NS_ASSUME_NONNULL_END